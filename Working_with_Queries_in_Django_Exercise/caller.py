...
def show_hard_dungeons():
    ...
    result = [f"{dungeon.name} is guarded by {dungeon.boss_name} who has {dungeon.boss_health} health points!" for
              dungeon in hard_dungeons]
    return '\n'.join(result)
...
def show_workouts():
    ...
    result = [f"{workout.name} from {workout.workout_type} type has {workout.difficulty} difficulty!" for workout in workouts]
    return '\n'.join(result)
...
def delete_workouts():
    Workout.objects.all().exclude(workout_type__in=('Strength', 'Calisthenics')).delete()
