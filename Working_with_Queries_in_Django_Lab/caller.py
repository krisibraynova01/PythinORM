...
def find_books_by_genre_and_language(genre, language):
    book_title = Book.objects.filter(genre=genre, language=language)
    return book_title
...
def find_authors_nationalities():
    ...
    result = [f"{nationality.first_name} {nationality.last_name} is {nationality.nationality}" for nationality in
              nationalities]
    return '\n'.join(result)
...
def order_books_by_year():
    order_books = Book.objects.order_by('publication_year', 'title')
    result = [f"{book.publication_year} year: {book.title} by {book.author}" for book in
              order_books]
    return '\n'.join(result)
...
def delete_review_by_id(review_id):
    id = Review.objects.get(id=review_id)
    result = f"Review by {id.reviewer_name} was deleted"
    id.delete()
    return result
...
def filter_authors_by_nationalities(nationality):
    ...
    result = [f"{author.biography}" if author.biography else f"{author.first_name} {author.last_name}" for author in authors]
    return '\n'.join(result)
